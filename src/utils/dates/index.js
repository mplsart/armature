...
export function sort_event_dates_by_start(ed1, ed2) {
 ...
 *
 */
export function get_best_event_date(event_resource, starting_date_filter, type_filter=null) {
  ...
  if (type_filter) {
    for (var i in sorted_event_dates) {
      ...
      if (sorted_event_dates[i].type == type_filter) {
        if ((type_filter == 'timed' && moment(sorted_event_dates[i].start) > starting_date_filter)
             || (type_filter != 'timed')) {
